...
def get_training_model():
    ...
    7 * len(setts.CHARS) vector, `v`. `v[0]` is the probability that a plate is
    fully within the image and is at the correct scale.

    `v[1 + i * len(setts.CHARS) + c]` is the probability that the `i`'th
    character is `c`.
    """
    ...
    # Output layer
    W_fc2 = weight_variable([2048, 1 + 7 * len(setts.CHARS)])
    b_fc2 = bias_variable([1 + 7 * len(setts.CHARS)])
...
def get_detect_model():
    ...
    # Fifth layer
    W_fc2 = weight_variable([2048, 1 + 7 * len(setts.CHARS)])
    W_conv2 = tf.reshape(W_fc2, [1, 1, 2048, 1 + 7 * len(setts.CHARS)])
    b_fc2 = bias_variable([1 + 7 * len(setts.CHARS)])
