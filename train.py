...
def code_to_vec(p, code):
    def char_to_vec(c):
        y = np.zeros((len(setts.CHARS),))
        y[setts.CHARS.index(c)] = 1.0
        return y
...
def train(learn_rate, report_steps, batch_size, initial_weights=None):
    ...
    """
    x, y, params = nn.get_training_model()

    y_ = tf.placeholder(tf.float32, [None, 7 * len(setts.CHARS) + 1])

    digits_loss, presence_loss, loss = get_loss(y, y_)
    train_step = tf.train.AdamOptimizer(learning_rate=learn_rate).minimize(loss)

    best = tf.argmax(tf.reshape(y[:, 1:], [-1, 7, len(setts.CHARS)]), 2)
    correct = tf.argmax(tf.reshape(y_[:, 1:], [-1, 7, len(setts.CHARS)]), 2)
    ...
    def vec_to_plate(v):
        return "".join(setts.CHARS[i] for i in v)
    ...
    def do_report():
        ...
        print ("Batch {:3d} {:2.02f}% {:02.02f}% loss: {} "
