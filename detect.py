...
def detect(im, param_vals):
    ...
    scaled_ims = list(make_scaled_ims(im, nn.WINDOW_SHAPE))

    # Load the nn which detects number plates over a sliding window.
    x, y, params = nn.get_detect_model()

    # Execute the nn at each scale.
    ...
    # Do this by identifying windows (at all scales) where the nn predicts a
        ...
        for window_coords in numpy.argwhere(y_val[0, :, :, 0] >
            ...
            letter_probs = (y_val[0,
                                  ...
                                  window_coords[1], 1:].reshape(
                                    7, len(setts.CHARS)))
            letter_probs = setts.softmax(letter_probs)

            img_scale = float(im.shape[0]) / scaled_im.shape[0]

            bbox_tl = window_coords * (8, 4) * img_scale
            bbox_size = numpy.array(nn.WINDOW_SHAPE) * img_scale

            present_prob = setts.sigmoid(
...
def letter_probs_to_code(letter_probs):
    return "".join(setts.CHARS[i] for i in numpy.argmax(letter_probs, axis=1))
